    ...
    <v-main class="app-main-content">
      <v-container fluid class="pa-4">
        <v-row>
          ...
          <v-col cols="12" md="4" class="available-elements-col" v-if="!$vuetify.display.mobile">
            <AvailableElements />
          </v-col>

          <!-- Form Builder Canvas and JSON Output Column -->
          <v-col cols="12" :md="$vuetify.display.mobile ? 12 : 8" class="form-builder-col">
            <v-card
            ...
            >
              ...
              <v-card-text>
                <p v-if="formBuilderStore.formElements.length === 0 && !isDragOver" class="empty-form-message">
                ...
                <div v-if="formBuilderStore.formElements.length > 0" class="elements-list mt-3">
                  ...
                  <ul>
                    <li
                      ...
                      :class="{'selected-element': formBuilderStore.selectedElementId === element.id}"
                      @click="selectElementForEditing(element.id)"
                    >
                      <v-icon :icon="getElementIcon(element.type)" class="mr-3"></v-icon>
                      <span class="element-label">{{ element.label || 'Unnamed Element' }} ({{ element.type }})</span>
                      <div class="element-actions">
                        <v-btn
                          size="small" variant="text" color="primary"
                          @click.stop="selectElementForEditing(element.id)"
                          title="Edit Element" icon="mdi-pencil-outline"
                        ></v-btn> <!-- Changed to text variant for less clutter -->
                        <v-btn
                          size="small" variant="text" color="error"
                          @click.stop="removeElement(element.id)"
                          title="Remove Element" icon="mdi-delete-outline"
                        ></v-btn> <!-- Changed to text variant -->
                      </div>
                    </li>
                  </ul>
                </div>
            ...
            <v-card class="mt-4 json-output-card">
              <v-card-title class="d-flex justify-space-between align-center">
                ...
                <v-btn color="secondary" @click="copyJsonToClipboard" prepend-icon="mdi-content-copy">
                  Copy JSON
                </v-btn>
              </v-card-title>
...
const snackbar = ref({
  show: false, text: '', color: 'success', timeout: 3000,
});
...
const handleDragLeave = ()_ => { isDragOver.value = false; };
...
const handleDrop = (event: DragEvent) => {
  ...
  if (event.dataTransfer) {
    ...
    try {
      ...
      const elementDef = elementDefinitions.find(def => def.type === elementType);
      if (elementDef) {
      ...
      } else { console.error('Dropped element type not found:', elementType); }
    } catch (e) { console.error('Failed to parse dropped data:', e); }
  }
};
...
const transformedFormJson = computed(() => {
  return formBuilderStore.formElements.map(el => {
    const outputEl: Record<string, any> = {
      id: el.id, tipo: el.type, etiqueta: el.label, valor: el.value,
      nombreVariable: el.variableName, placeholder: el.placeholder, textoAyuda: el.hint,
      obligatoriedad: el.requirementLevel, capitulo: el.chapter, pregunta: el.question,
      numeroPregunta: el.questionNumber, condicionConsistencia: el.consistencyCondition,
      mensajeInconsistencia: el.inconsistencyMessage, tipoError: el.errorType,
      descripcion: el.description, reglas: el.rules, nombre: el.name,
      deshabilitado: el.disabled, soloLectura: el.readonly,
      subtipo: el.specificType, // For text field's type like 'number', 'email'
    };
    if (el.type === 'select' || el.type === 'radio-group') { outputEl.opciones = el.options; }
    if (el.type === 'textarea' && el.height) { outputEl.altura = el.height; }
    if (el.type === 'button' && el.color) { outputEl.color = el.color; }


    Object.keys(outputEl).forEach(key => {
      if (outputEl[key] === undefined || outputEl[key] === null || (Array.isArray(outputEl[key]) && outputEl[key].length === 0)) {
        if (outputEl[key] !== '' && outputEl[key] !== false) { // Keep empty strings and false booleans
             delete outputEl[key];
        }
      }
    });
...
const transformedFormJsonString = computed(() => JSON.stringify(transformedFormJson.value, null, 2));

const copyJsonToClipboard = async () => {
  try {
    await navigator.clipboard.writeText(transformedFormJsonString.value);
    snackbar.value = { show: true, text: 'JSON copied to clipboard!', color: 'success', timeout: 3000 };
  } catch (err) {
    console.error('Failed to copy JSON: ', err);
    snackbar.value = { show: true, text: 'Failed to copy JSON.', color: 'error', timeout: 3000 };
  }
};
...
.drop-zone.drag-over { background-color: #e3f2fd; border-color: #2196f3; } /* Lighter blue, primary border */

.empty-form-message, .drop-prompt-message {
  text-align: center; color: #757575; font-style: italic; margin-top: 20px; padding: 10px;
}

.elements-list { margin-top: 15px; }
.form-element-item {
...
.element-label { flex-grow: 1; font-size: 0.9rem; } /* Slightly smaller label */
.element-actions { display: flex; align-items: center; }
